    ...
    NioApp.Dropzone = function (elm, opt) {
        if ($(elm).exists()) {
            $(elm).each(function (item) {
                var maxFiles = $(elm)[item].dataset.maxFiles, maxFiles = maxFiles ? Number(maxFiles) : null;
                var maxFileSize = $(elm)[item].dataset.maxFileSize, maxFileSize = maxFileSize ? Number(maxFileSize) : 256;
                var acceptedFiles = $(elm)[item].dataset.acceptedFiles, acceptedFiles = acceptedFiles ? acceptedFiles : null;
        ...
        if ($(elm).exists()) {
            $(elm).each(function () {
                ...
                var def = {
                    ...
                    attr = (opt) ? extend(def, opt) : def;
                attr = (auto_responsive === false) ? extend(attr, { responsive: false, autoWidth: false }) : attr;
